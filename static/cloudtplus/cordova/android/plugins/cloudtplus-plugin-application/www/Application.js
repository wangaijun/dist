cordova.define("cloudtplus-plugin-application.Application", function(require, exports, module) {
     ...
     *  调用加入人脸识别的相机
     */
    applicationExport.scanFace = function(successCallback, errorCallback) {
        return exec(successCallback, errorCallback, "Application", "getPicture", [{
                    sourceType: 0, // 0:拍照 1:系统相册
                    maxCount: 1, // 0:表示无限制
                    maxBurstCount: 0, // 0:表示无限制 // 弃用maxBurstCount
                    encodingType: 0,			// 0:JPEG 1:PNG
                    targetWidth: undefined,		// 照片宽度，保持宽高比
                    targetHeight: undefined,	// 照片高度，保持宽高比
                    quality: 60,				// 图像质量:0~100
                    cameraDirection: 0,			// 0:后置摄像头 1:前置摄像头
                    allowVideo: false,	// true:拍照时允许录制短视频
                    maxDuration: 8000,	// 限制录制时长，默认8秒
                    watermark: "",    // 在照片上添加的水印文字
                    waterColor: "",    // 水印文字颜色，默认为白色
                    cropRate: 1     // 剪裁图片的剪裁框的宽高比，默认为0，表示不剪裁
                }]);
    };
